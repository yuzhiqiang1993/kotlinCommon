 ...
 */
fun FragmentActivity.getPermissions(
    ...
    XXPermissions.with(this).permission(permissions).request(object : OnPermissionCallback {
        ...
        override fun onDenied(deniedPermissions: MutableList<String>, doNotAskAgain: Boolean) {
            ...
            if (doNotAskAgain) {/*用户点了拒绝并勾选了不在提示 弹窗提示用户手动给权限*/
                showPermissionSettingDialog(
                    deniedPermissions, permissionDenide, permissionGranted
                )
            } else {/*申请权限时用户没给权限，但是也没选择不再询问，例如申请后台定位权限，没给始终允许直接返回了，给用户提示选择始终允许*/
...
private fun specialPermissionPrompt(deniedPermissions: MutableList<String>) {
    ...
    if (deniedPermissions.size == 1) {
        ...
        if (Permission.ACCESS_BACKGROUND_LOCATION == deniedPermission) {/*如果是申请后台定位权限 提示用户勾选始终允许*/
            Toast.makeText(
                AppContext,
 ...
 * @receiver
 */
fun FragmentActivity.showPermissionSettingDialog(
    ...
    PromptDialog(this).apply {
        content(getHintMessage(deniedPermissions))
    }.positiveBtn("去设置") { v ->
        /*跳转到设置页面*/
        XXPermissions.startPermissionActivity(
            this, deniedPermissions, object : OnPermissionPageCallback {
                override fun onGranted() {/*在设置页面都给权限了 执行逻辑*/
                    permissionGranted()
                }
...
private fun ComponentActivity.getHintMessage(
    ...
    if (deniedPermissions.size == 1) {
        ...
        if (Permission.BODY_SENSORS_BACKGROUND == deniedPermission) {/*传感器权限只能选择始终允许*/
