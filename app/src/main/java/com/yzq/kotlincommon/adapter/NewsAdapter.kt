...
class NewsAdapter(layoutResId: Int, data: List<NewsBean.Data>, val context: BaseActivity) : BaseQuickAdapter<NewsBean.Data, BaseViewHolder>(layoutResId, data) {


    override fun convert(helper: BaseViewHolder, item: NewsBean.Data) {

        helper.itemView.tv_title.text = item.title
        ImageLoader.loadCenterCrop(context, item.thumbnailPicS, helper.itemView.iv_img, radius = 15)
